...
- 5 个竞品审计面板（Typefully / Tweet Hunter / Hypefury / Buffer / Metricool）
- 用户高频操作流程洞察（预热、首波互动、拆分回流、复盘闭环）
- 多维规则推演模型（写作匹配 / 分发潜力 / 合规安全 / 转化潜力 / 执行效率）
- 独立 Markdown 排版工坊（`/md-lab`）：参考 doocs/md 思路做二次创作，支持公众号/博客/知乎等长文排版
- 支持从主工具一键读取草稿到排版工坊，并导出富文本 / HTML / Markdown
- 新增文案卡片工具套件（`/card-suite`）：含文案工场、卡片设计、发布中心、策略面板四个页面
- 支持长文草稿共享到卡片设计，自动拆分多张卡片并生成多平台发布文案
...
12. 点击 `Markdown 排版工坊` 进入 `/md-lab` 做二次排版与跨平台长文输出。
13. 点击 `文案卡片工具套件` 进入 `/card-suite`，执行「文案生成 → 卡片拆分 → 发布输出」流程。
